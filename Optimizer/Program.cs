...
namespace Optimizer
{
    internal static class Program
    {
        ...
        /// </summary>
        internal static readonly float Major = 16;

        internal static readonly float Minor = 4;
        internal static readonly bool EXPERIMENTAL_BUILD = false;
        ...
        private const string _jsonAssembly = @"Optimizer.Newtonsoft.Json.dll";

        internal static MainForm _MainForm;
        internal static SplashForm _SplashForm;

        private static string _adminMissingMessage = "Optimizer needs to be run as administrator!\nApp will now close...";
        private static string _unsupportedMessage = "Optimizer works with Windows 7 and higher!\nApp will now close...";

        private static string _confInvalidVersionMsg = "Windows version does not match!";
        private static string _confInvalidFormatMsg = "Config file is in invalid format!";
        private static string _confNotFoundMsg = "Config file does not exist!";
        private static string _argInvalidMsg = "Invalid argument! Example: Optimizer.exe /config=win10.json";
        private static string _alreadyRunningMsg = "Optimizer is already running in the background!";

        private const string MUTEX_GUID = @"{DEADMOON-0EFC7B8A-D1FC-467F-B4B1-0117C643FE19-OPTIMIZER}";
        internal static Mutex MUTEX;
        private static bool _notRunning;
        ...
        static void Main(string[ ] switches)
        {
            ...
            if(switches.Length == 1)
            {
                ...
                if(arg.StartsWith("/svchostsplit="))
                {
                    ...
                    if(isValid && int.TryParse(x , out int result))
                        Utilities.DisableSvcHostProcessSplitting(result);
                    Environment.Exit(0);
                    return;
                }
                ...
                if(arg == "/version")
                {
                    if(!EXPERIMENTAL_BUILD)
                        MessageBox.Show($"Optimizer: {GetCurrentVersionTostring( )}\n\nCoded by: deadmoon © ∞\n\nhttps://github.com/hellzerg/optimizer" , "Optimizer" , MessageBoxButtons.OK , MessageBoxIcon.Information);
                    else
                        MessageBox.Show("Optimizer: EXPERIMENTAL BUILD. PLEASE DELETE AFTER TESTING.\n\nCoded by: deadmoon © ∞\n\nhttps://github.com/hellzerg/optimizer" , "Optimizer" , MessageBoxButtons.OK , MessageBoxIcon.Information);
                ...
                if(arg.StartsWith("/disable="))
                {
                    ...
                    if(opts.Contains(Constants.INDICIUM_TOOL))
                        o1 = true;
                    else
                        o1 = null;
                    if(opts.Contains(Constants.UWP_TOOL))
                        o2 = true;
                    else
                        o2 = null;
                    if(opts.Contains(Constants.APPS_TOOL))
                        o3 = true;
                    else
                        o3 = null;
                    if(opts.Contains(Constants.HOSTS_EDITOR))
                        o4 = true;
                    else
                        o4 = null;
                    if(opts.Contains(Constants.STARTUP_TOOL))
                        o5 = true;
                    else
                        o5 = null;
                    if(opts.Contains(Constants.CLEANER_TOOL))
                        o6 = true;
                    else
                        o6 = null;
                    if(opts.Contains(Constants.INTEGRATOR_TOOL))
                        o7 = true;
                    else
                        o7 = null;
                    if(opts.Contains(Constants.PINGER_TOOL))
                        o8 = true;
                    else
                        o8 = null;
